...
(function () {
  ...
  function generateHeadingId(text) {
    return 'toc-' + text.toLowerCase().trim().replace(/[^\w\s-]/g, '') // Remove special characters
  ...
  function addHeadingIds() {
    ...
    const postContent = document.querySelector('.entry-content, .wp-block-post-content');
    if (!postContent) {
      return;
    }
  ...
  function setupScrollTracking() {
    ...
    function setActiveLink(headingId) {
      ...
      if (headingId) {
        const activeLink = document.querySelector(`.toc-item[href="#${headingId}"]`);
        if (activeLink) {
    ...
    const observerOptions = {
      ...
      // Top offset for header, bottom threshold.
      threshold: 0
    };
  ...
  function init() {
    ...
    const tocBlock = document.querySelector('.wp-block-fse-pilot-table-of-contents');
    if (!tocBlock) {
      return;
    }
    addHeadingIds();
